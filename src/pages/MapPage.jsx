...
import { getMapItems } from "../data/mapData";
    ...
    <div className="mb-12">
      <h2 className="font-display text-2xl font-semibold mb-6 flex items-center gap-3 ">
        ...
        <span className="text-slate-500 text-lg">
          ({items.filter((item) => unlockedIds.includes(item.id)).length}/
          {items.length})
        </span>
      </h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {sortedItems.map((item) => {
          ...
          const iconUrl =
            item.icon ||
            (item.signature && item.file_id
              ? gw2Api.getIconUrl(item.signature, item.file_id)
              : null);
...
export default function MapPage() {
  ...
  useEffect(() => {
    async function loadMapData() {
      ...
      try {
        ...
        const mapConfig = getMapItems(mapId);
        if (!mapConfig) {
        ...
        const [
          ...
          gw2Api.getSkins(mapConfig.skins),
          gw2Api.getMinis(mapConfig.minis),
          gw2Api.getNovelties(mapConfig.novelties),
          gw2Api.getAchievements(mapConfig.achievements),
        ...
        setUnlockedNovelties(
          Array.isArray(accountNovelties) ? accountNovelties : [],
        );
  ...
  if (!mapInfo) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-3xl font-display font-bold text-slate-300 mb-4">
            Map Not Found
          </h1>
  ...
  const totalItems =
    skins.length + minis.length + novelties.length + achievements.length;
  const unlockedItems =
        ...
        <div className="mb-12">
          <div className="h-3 bg-slate-800 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${totalItems > 0 ? (unlockedItems / totalItems) * 100 : 0}%`,
              }}
            />
          </div>
        </div>
